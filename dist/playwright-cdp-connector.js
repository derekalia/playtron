...
exports.PlaywrightCDPConnector = void 0;
...
class PlaywrightCDPConnector {
    constructor() {
        ...
        this.tabPageMap = new Map(); // Track tabs by their ID
    ...
    async connect() {
        ...
        try {
            ...
            this.browser = await playwright_1.chromium.connectOverCDP(this.cdpEndpoint);
            ...
            console.error('[PlaywrightCDP] ❌ Failed to connect to Electron browser:', error.message);
     ...
     */
    async getAccessibilitySnapshot(options) {
        if (!this.page) {
     ...
     */
    async getSimplifiedPageStructure() {
        if (!this.page) {
            throw new Error('No page connected');
        }
     ...
     */
    simplifyAccessibilityNode(node, depth = 0) {
        const results = [];
        if (!node)
            return results;
        ...
        if (node.multiselectable !== undefined)
            simplified.multiselectable = node.multiselectable;
        if (node.required !== undefined)
            simplified.required = node.required;
        if (node.readonly !== undefined)
            simplified.readonly = node.readonly;
    ...
    async createTab(url = 'https://www.google.com') {
        ...
        try {
            ...
            const tabId = await mainPage.evaluate((url) => {
                const win = window;
...
exports.PlaywrightCDPConnector = PlaywrightCDPConnector;
//# sourceMappingURL=playwright-cdp-connector.js.map